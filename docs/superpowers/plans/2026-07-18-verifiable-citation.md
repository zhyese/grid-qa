...
**前端（1）：** `frontend/src/views/Chat.vue` 引用卡片 + 警示标签（bbox 高亮本期仅页码锚定，YAGNI 重渲染）。
...
**Files:**
- Modify: `frontend/src/views/Chat.vue`（引用渲染区）
- Modify: `frontend/src/api/qa.js`（透传新字段，若有类型声明）

**Interfaces:**
- Consumes: 后端返回 `citationMap`（每项含 `metadata.doc_title/section_path/page_num/original_text`）、`unverifiedClaims`、`citationIndex`。
- 产出：点击 `[n]` → 弹出来源卡片（文档名/章节/页码/完整原文片段）；`unverifiedClaims` 句前加浅色「⚠ 未核验」标签。

- [ ] **Step 1: Read Chat.vue 现有答案渲染区**

定位答案 markdown 渲染 + retrievalSource 卡片现有结构（`frontend/src/views/Chat.vue`，搜索 `retrievalSource` / `citation`）。**不重写**，在现有来源卡片旁加引用元数据展示。

- [ ] **Step 2: 加引用卡片组件片段**

在 Chat.vue 的来源展示区加（遵循现有 Vue 3 Composition + `hasPerm` 惯例）：

```vue
<!-- 可核验引用卡片：点击 [n] 展示精确来源 -->
<div v-if="msg.citationMap?.length" class="citation-cards">
  <span v-for="c in msg.citationMap" :key="c.ref_id" class="citation-card"
        @click="openCitation(c)">
    [{{ c.ref_id }}]
  </span>
</div>
<div v-if="msg.unverifiedClaims?.length" class="unverified-banner">
  <span v-for="(u, i) in msg.unverifiedClaims" :key="i" class="badge badge-warning">⚠ 未核证：{{ u.slice(0, 30) }}…</span>
</div>
```

```javascript
function openCitation(c) {
  // 弹窗展示完整元数据 + 锚定预览页（bbox 高亮本期仅页码锚定，YAGNI 重渲染）
  const m = c.metadata || {}
  toast(`[${c.ref_id}] ${m.doc_title || ''} · ${m.section_path || ''} · 第${m.page_num || '?'}页`)
  // 若有 docId + page_num，可调 /document/preview/{docId} 跳页（现有接口）
}
```
...
```css
.citation-cards { margin-top: 6px; }
.citation-card { color: #2563eb; cursor: pointer; margin-right: 2px; }
.citation-card:hover { text-decoration: underline; }
.unverified-banner { margin-top: 4px; }
```

- [ ] **Step 4: 手动验证（前端在跑）**

启动前端（已 5173 端口），提问 → 答案末尾 `[n]` 可点击弹出来源；开关关时不出现卡片。截图确认。

- [ ] **Step 5: 提交**

```bash
git add frontend/src/views/Chat.vue frontend/src/api/qa.js
git commit -m "feat(citation): 前端引用卡片(章节/页码/原文)+未核验警示标签"
```
...
| §3.5 第五层 前端 + 评测 | Task 11（前端）+ Task 12（评测） ✅ |
