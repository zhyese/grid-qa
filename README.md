...
graph TB
    ...
    %% ===== 问答读取流：缓存 → 检索 → CRAG自纠错 → 图谱 → 重排 → 生成 =====
    BE -->|"⑤ 命中热点?"| Redis
    BE -->|"query → 向量"| EMB
    BE -->|"稠密 TopK"| MilvusC
    BE -->|"稠密 TopK"| MilvusB
    BE -->|"候选 → 重排"| RR
    RR -.->|"rerank分 → CRAG分级"| CRAG["★ CRAG 自纠错<br/>correct/ambiguous/incorrect<br/>incorrect→改写重检索→refused"]
    BE -.->|"⑥ 图谱上下文"| Neo4j
    BE -->|"⑦ prompt + 置信度"| LLM
...
flowchart LR
    ...
    subgraph R["② 读取 · GraphRAG + CRAG 自纠错问答融合"]
        Q[提问] --> CC{"热点缓存<br/>命中?"}
        CC -->|命中| A1[秒回 cached]
        CC -->|未命中| RT[检索<br/>BM25 + 双路向量]
        RT --> RRK[rerank 重排]
        RRK --> CG{"★CRAG分级<br/>rerank分"}
        CG -->|correct| PF["prompt<br/>文档+图谱"]
...
sequenceDiagram
    ...
    alt 命中
        ...
        BE->>BE: + BM25 → RRF 融合
        BE->>LLM: rerank 重排候选
        BE->>BE: ★ CRAG 分级(rerank分数)
        alt incorrect(低相关)
            BE->>LLM: 改写 query 重检索
            BE->>BE: 再分级→仍低=refused
        end
        BE->>Neo4j: graph_context 三元组
        BE->>LLM: prompt(文档+图谱+置信度) 流式
        loop 逐 token
            LLM-->>BE: token
            BE-->>FE: SSE token
        end
        BE->>MySQL: 存对话消息
        BE->>Redis: 写热点缓存(TTL)
        BE-->>FE: done(图谱N·confidence·引用·耗时)
    end
```
